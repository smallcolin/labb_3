...
var app = angular.module('Labb3', ['ui.router', 'chart.js']);
	...
	app.controller('First', ["$rootScope", function($rootScope) {
...
// CITIES SECTION
	...
	app.controller('Joncit', ["$rootScope", "cities", function ($rootScope, cities) {
	    $rootScope.cities = cities.data.items;
	}]);

	app.controller('popName', ["$scope", function($scope) {
	}]);
...
// CHARTS
	...
	app.controller("BaseCtrl", ["$scope", "$rootScope", function ($scope, $rootScope) {
		$scope.labels = [];
		$scope.data = [];
...
//  MATHS SECTION
	...
	app.factory('addition' , ["addNumbers", function (addNumbers) {
	...
	app.controller('FourController', ["$scope", "addNumbers", "addition", function( $scope, addNumbers, addition ) {
		addition.add();
